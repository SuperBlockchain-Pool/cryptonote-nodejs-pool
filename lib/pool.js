...
if (config.cnAlgorithm === "yespower_blake256") {
	noncePattern = new RegExp("^[0-9a-f]{8}$");
}
...
if (config.cnAlgorithm === "yespower_blake256") {
	multiHashing = require("multi-hashing-dogemone");
}
...
} else if (config.cnAlgorithm === "yespower_blake256") {
	cryptoNight = multiHashing.yespower_blake256;
...
function BlockTemplate (template, parent, indexOfChildPool) {
	...
	instanceId.copy(this.buffer, this.reserveOffset + 4, 0, (config.reserveBytes || 4));
...
Miner.prototype = {
	retarget: function (now) {
		...
		if (avg > VarDiff.tMax && this.difficulty > options.minDiff) {
			...
			if (config.coin === "Zent") {
				newDiff = newDiff > 1000000 ? newDiff : 1000000;	
			}
			direction = -1;
