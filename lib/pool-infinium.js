...
function handleMinerMethod(socket, method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){   
        ...
        case 'login':
            ...
            let permanentBan = config.poolServer.permanentBan;
            let permanentBanApply = null;

            for (let apply in permanentBan) {
                if (login.includes(apply)) {
                    permanentBanApply = permanentBan[apply];
                  break;
                }
            }

            if (permanentBanApply !== null) {
                sendReply(permanentBanApply);
            }
